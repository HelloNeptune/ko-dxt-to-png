  ...
  return new Promise((resolve, reject) => {
    fs.readFile(file, (err, buffer) => {
      ...
      try {
        if (
          buffer
            .slice(buffer.length - tgaSignature.length - 1, buffer.length - 1)
            .toString() == tgaSignature
        ) {
          ...
          png
            .pack()
            .pipe(fs.createWriteStream(path.resolve(outputLocation)))
        ...
        if (nameLength > 200) {
          return resolve({
            status: false,
            message: new Error(
              "name cannot be longer than 200, probably this file is invalid, len: " +
                nameLength
            ),
          });
        }
        ...
        //   return reject(
        //     new Error(
        //       "this game is too old, cannot have this much texture width: " +
        //         width +
        //         ", height: " +
        //         height
        //     )
        ...
        if (typeData.toString().indexOf("DXT1") != -1) {
          ...
          return resolve({
            status: false,
            message: new Error("Unsupported file! [" + typeData.join(", ") + "]"),
          });
        }
        ...
        console.error("[File couldn't converted", e);
        return;
      }
    });
  });
